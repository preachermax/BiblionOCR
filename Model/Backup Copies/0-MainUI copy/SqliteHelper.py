import sqlite3

class SqliteHelper:

    def __init__(self,name=None):
        self.conn = None
        self.cursor = None

        if name:
            self.open1(name)

    def open1(self,dbase1):
        try:
            self.conn = sqlite3.connect(dbase1)
            self.cursor = self.conn.cursor()
            print(sqlite3.version)
        except sqlite3.Error as e:
            print("Failed connecting to database...")

    '''def create_table(self):#CREATE
        c = self.cursor
        c.execute("""CREATE TABLE users(
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    name TEXT NOT NULL,
                    year INTEGER,
                    admin INTEGER
                )""")'''
    
    def open2(self,dbase2):#OPEN
        try:
            self.conn = sqlite3.connect(dbase2)
            self.cursor = self.conn.cursor()
            print(sqlite3.version)
        except sqlite3.Error as e:
            print("Failed connecting to database...")
    
    def selectone(self,query,data):#SELECT ONE
        c = self.cursor
        c.execute(query,data)
        return c.fetchone()
    
    def select(self,query):#SELECT
        c = self.cursor
        c.execute(query)
        return c.fetchall()
    
    def insert(self,query,data):#INSERT
        c = self.cursor
        c.execute(query,data)
        self.conn.commit()   
 
    def update(self,query,data):#UPDATE
        c = self.cursor
        c.execute(query,data)
        self.conn.commit()

    def delete(self,query):#DELETE
        c = self.cursor
        c.execute(query,data)
        self.conn.commit() 


        

#test = SqliteHelper("test.db")
#test.create_table()

#test.edit("INSERT INTO users (name,year,admin) VALUES ('john',1992,0) ")
#test.edit("UPDATE users SET name='jack' WHERE name = 'john'")
#print(test.select("SELECT * FROM users"))