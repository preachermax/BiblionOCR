...
* 2026-07-09: `Core/compute_profile.py` now defines schema-only dataclasses for the normalized compute hardware profile surface (`CPUProfile`, `MemoryProfile`, `StorageProfile`, `GPUProfile`, `CUDAProfile`, `ProviderProfileEntry`, and `HardwareProfile`) without adding provider logic or engine integration yet
