...
# Application Viewdef __init__(self, imgpath=None, parent=None):
    def __init__(self, imgpath=None, parent=None):
        ...
        # -------------------------
        # Phase 2.5 — PATH SANITY GUARD (DEBUG)
        # -------------------------
        def _assert_clean_path(label, path):
            if path and os.path.isabs(path):
                # Detect accidental double-prefix like: projecthome + absolute path
        ...
        # -------------------------
        # Phase 6 — SAFE AUTO-LOAD FLAG
        # -------------------------
        
        assert not (self.refimgpath and "C:/" in self.refimgpath and "BiblionOCRC:" in self.refimgpath), "PATH CORRUPTION DETECTED"
        ...
        if self.refimgpath and os.path.isfile(self.refimgpath):
            ...
            self._startup_load = True
        else:
            print("[INIT] No valid startup image")
            self._startup_load = False

        # -------------------------
        # Phase 7 — DEFERRED LOAD (AFTER UI READY)
        # -------------------------
        if self._startup_load:
            print("[INIT] Triggering deferred image load")
            self.setupRefImages()

        if self._startup_load:
            qtc.QTimer.singleShot(0, lambda: self.showRefImg(self.refimgpath))
...
# Session View
    ...
    def get_session_settings(self):
        ...
        imgpath = scanner_session.get('self.imgpath', getattr(self, 'imgpath', ''))
        imgdir = scanner_session.get('self.imgdir', getattr(self, 'imgdir', ''))

        # ✅ normalize ONLY — do NOT join with projecthome
        self.imgpath = os.path.normpath(imgpath) if imgpath else ""
        self.imgdir = os.path.normpath(imgdir) if imgdir else ""
...
# Application Controllers
    ...
    def importRefImg(self):
        ...
        if self.imgpath:
            ...
            print("[Pixler] Ref image indexed, deferring load")
