...
2. Human developers use `Developer/Developer.py` as the required guided implementation of this checklist instead of manually tracking this document.
3. The wizard preserves developer choice over design and implementation while requiring evidence for every universal gate and a written justification for every conditional gate marked not applicable.
4. Human-developer pull requests are eligible for review only when the wizard reports `PR ELIGIBLE` and its generated evidence block passes `.github/workflows/enforce-pr-checklist.yml`.
5. If manual verification is required (for example visual UI checks), the responsible developer or agent records the result in the workflow report and handoff checkpoint.
...
2. Regenerate corresponding `ViewController/**/*UI.py` with `Developer/update_ui_resources.py` or its platform wrapper; the utility must replace only outputs whose generated bytes changed.
3. Verify no drift remains between `.ui` and generated `UI.py`.
   - If a generated module has a documented deterministic analysis header added by the canonical regeneration command, compare the generated body after that header with fresh `pyuic5` output.
