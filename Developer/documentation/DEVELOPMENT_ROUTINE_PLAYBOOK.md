...
3. Keep session files only in `Model/Project/Data/json`; never recreate a mirrored `ViewController/Model` tree.
...
Before pause/exit, the agent writes a restart-safe handoff in `Developer/documentation/DEV_NOTEBOOK.md` containing:
...
1. read `Developer/documentation/DEVELOPMENT_ROUTINE_CHECKLIST_ONE_PAGE.md`
2. read this playbook
3. read latest `Session Handoff` entry in `Developer/documentation/DEV_NOTEBOOK.md`
