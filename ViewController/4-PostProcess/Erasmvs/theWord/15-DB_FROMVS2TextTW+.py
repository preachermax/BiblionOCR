...
while loopcount <= lastline:
    cursor_TW.execute("SELECT Line,Word,Strong,RMAC,Lemma,WordPunc FROM Bible WHERE Line = " + str(loopcount))
    ...
    for vword in vwords:
        if vword:
            ...
            wordgroup = wordopen + str(word) + strongopen + str(strong) + strongclose + rmacopen + str(RMAC) + rmacclose + lemmaopen + str(lemma) + lemmaclose + str(punc)
