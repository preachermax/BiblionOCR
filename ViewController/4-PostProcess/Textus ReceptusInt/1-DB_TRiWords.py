...
conn_TR = sqlite3.connect('/home/max/Projects/Python/SQLite/TRiBible.db')
...
#If you want to exclude just space and newline characters, then you might want to use r'^[^ \n]*$'
    ...
    pattern = re.compile('''([.;,?!-··::'"()]?\w+[.;,?!-··::'"()]?'?\s?\s)(([.;,?!-··::'"()]?\w+[.;,?!-···::'"()]?'?s?\s?[.;;,?!-··::'"()]?\s)*)(G\d+\s)(\w+-?(\w+)?-?(\d)?(\w+)?)''')
    #pattern = re.compile('''((\w+\s?[.;,?!-··::'"]?\s){2,})(\w+-?(\w+)?-?(\d)?(\w+)?)''')
    #pattern = re.compile('([^ \n,.;]+)([,.;]?)')
    #pattern = re.compile('([^ \n,.;]+)([,.;]?[^ \n\s])')
    ...
    for match in matches:
        #print(match)
        mstr = match.group(1).split(" ")
        wordstr = mstr[0]
        '''puncpat = re.compile('(/w+)([.;,·])?')
        puncmatches = puncpat.finditer(wordstr)
        for puncmatch in puncmatches:
            if puncmatch[1]:
                punc = puncmatch[1]
            else:
                punc = ""
            word = puncmatch[0]'''
        if wordstr[-1] == "," or wordstr[-1] == ";" or wordstr[-1] == "." or wordstr[-1] == "·":
            punc = wordstr[-1]
            word = wordstr[:-1]
        else:
            punc = ""
        ...
        #englishlist = mstr[1:-2]
        #english = "  ".join([str(item) for item in englishlist])
        english = match.group(2)
        strong = match.group(4).strip()
        rmac = match.group(5)
        #print(word)
        ...
        insert_parameters = """INSERT OR REPLACE INTO Bible (ID,Line,Book,Chapter,Verse,WordNum,Word,Strong,RMAC,English,WordPunc)
                            VALUES ((select ID from Bible WHERE Line=? AND WordNum=?),?,?,?,?,?,?,?,?,?,?)"""
        insert_data = (linenum,wordnum,linenum,book,chpt,vrse,wordnum,word,strong,rmac,english,punc)
        ...
        print(linenum, "\t", word,"\t",english,"\t",strong,"\t",rmac)
        #open test.txt file
