...
class MainWindow(qtw.QMainWindow):
    ...
    # Start of LineBox Methods
   
    # Slots
    def on_cellSelectionChanged(self):
        self.ui.BoxTable.selectionModel().selectionChanged.disconnect(self.on_cellSelectionChanged)
        self.ui.BoxTable.setSortingEnabled(False)
        colcount = self.ui.BoxTable.columnCount()
        self.column_selected = self.ui.BoxTable.currentColumn()
        self.tableitem = self.ui.BoxTable.item(self.row_selected, self.column_selected)
        self.cellwidget = self.ui.BoxTable.cellWidget(self.row_selected, self.column_selected)
        self.cellvalue = self.ui.BoxTable.item(self.row_selected,self.column_selected).text()
        print(f'Selected Cell Location:  Row: {self.row_selected} Column: {self.column_selected}')
    ...
    def on_rowEditSelection(self):
        ...
        def tableEdit():
            ...
            #self.ui.BoxTable.cellClicked.connect(self.on_itemSelectionChanged)
        ...
        if popup.clickedButton() == mouseButton:
            ...
            pass
        #self.ui.BoxTable.selectionModel().currentChanged.connect(self.on_rowSelectionChanged)
    
    '''
    def on_rowSelectionChanged(self):
        #print(f'currentChanged event returned paramters: {self.ui.BoxTable.selectionModel().currentChanged()}')
        #self.ui.BoxTable.selectionModel().select(self.ui.BoxTable.selectionModel().currentIndex())
        self.ui.BoxTable.setSortingEnabled(False)
        self.row_selected = self.ui.BoxTable.currentRow()        
        print("Editing BoxTable selection")
    ...
    def BoxTable2csv(self):
        ...
        if os.path.exists(self.txtpath):
            ...
            self.ui.BoxText.clear()
        colCount = range(self.ui.BoxTable.columnCount()- 5)
            ...
            for row in range(0,rowcount):
                for col in range(0,colcount):
                    if col == 5:
                        ...
                        self.inslocation = "above"
                        #insertAButton.clicked.connect(self.insert_Row)
                    elif col == 6:
                        deleteButton = qtw.QPushButton(self.ui.BoxTable)
                        deleteIcon = qtg.QIcon()
                        deleteIcon.addPixmap(qtg.QPixmap(":/Icons/Icons/cross.png"), qtg.QIcon.Normal, qtg.QIcon.Off)
                        deleteButton.setIcon(deleteIcon)
                        ...
                        #rDrawButton.clicked.connect(self.rDrawEdit())
                    elif col == 8:
                        sDrawButton = qtw.QPushButton(self.ui.BoxTable)
                        sDrawIcon = qtg.QIcon()
                        sDrawIcon.addPixmap(qtg.QPixmap(":/Icons/Icons/table.png"), qtg.QIcon.Normal, qtg.QIcon.Off)
                        sDrawButton.setIcon(sDrawIcon)
                        self.ui.BoxTable.setCellWidget(row,col,sDrawButton)
                        #sDrawButton.clicked.connect(self.sDrawEdit())
                    elif col == 9:
    ...
    def insert_Row(self):
        row = self.ui.BoxTable.currentRow()
        if row:
            if self.inslocation == "above":
                self.ui.BoxTable.insertRow(row)
            elif self.inslocation == "below":
