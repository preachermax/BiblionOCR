...
fontcsvFilePath = fontcsvdir + fontfamily + '_' + fonttablename + '_splines.csv'
fontjsonFilePath = fontjsondir + fontfamily + '_' + fonttablename + '_splines.json'

cols = ["name","contour","point","x", "y", "on"]
rows = []
        ...
        if TTGlyph.attrib['name'] != ".null" and TTGlyph.attrib['name'] != "glyph2" and TTGlyph.attrib['name'] != "space"\
            ...
            for contour in TTGlyph.iter('contour'):    
                ...
                for pt in contour.iter('pt'):
                    ...
                    y = pt.attrib['y']
                    on = pt.attrib['on']
                    print(f'name {name}: contour {cnt} point {point} x: {x}, y: {y} on: {on}')
                    rows.append({"name": name, "contour": cnt, "point": point, "x": x, "y": y, "on": on})
                    point += 1
                cnt += 1
# Write ttx to csv
def table2_csv():
