...
from ImageTextPairDialog import Ui_ImageTextPairDialog

import MyPixer as pixer
...
# Menu and Toolbar Action Methods 
    ...
    '''def actionPixer(self):

        self.PixerWindow = qtw.QMainWindow()
        self.pixerui = pixer.Ui_Pixer()
        self.pixerui.setupUi(self.PixerWindow)
        self.PixerWindow.show()

        self.pixerui.OpenRefImgbutton.clicked.connect(self.loadRefImg)
        self.pixerui.ImportRefImgFilebutton.clicked.connect(self.importRefImg)
        self.pixerui.OverwriteRefImgbutton.clicked.connect(self.OverwriteRefImg)
        self.pixerui.ExportImageFilebutton.clicked.connect(self.ExportImage)
        self.pixerui.SaveImagebutton.clicked.connect(self.SaveImage)
        self.pixerui.SaveAsImagebutton.clicked.connect(self.SaveImageAs)
        #self.pixerui.OpenImageFilebutton.clicked.connect(self.OpenPixerFileDialog)
        #self.pixerui.PixerButton.clicked.connect(self.PixerTif(self.pixerui.Image))
        #self.pixerui.SavePixerpedImgAsbutton.clicked.connect(self.DestLatinDialog)
        #self.pixerui.SaveImagebutton.clicked.connect(self.DestLatinDialog)
        #self.pixerui.buttonBox.accepted.connect(accept)
        #self.pixerui.buttonBox.rejected.connect(reject)




        rsp = self.PixerWindow.exec_()'''   
    ...
    def actionPixer(self):
        print("Opening image in Pixer")
        self.PixerMain = qtw.QMainWindow()
        self.pixerui = pixer.Ui_Pixer()
        self.pixerui.setupUi(self.PixerMain)
        
        #cropimg.w = cropimg.MainWindow()
        if self.imgpath != "":
            self.pixerui.imgpath = self.imgpath
            self.pixerui.origpixmap = self.origpixmap
            self.pixerui.scale = 0.1
            self.pixerui.RefImg.setPixmap(self.origpixmap)
            self.pixerui.RefImgLE.setText(self.imgpath)
        #self.pixerui.show()
        self.PixerMain.show()
        #rsp = self.PixerWindow.main.show()
        print("Return from Pixer")
    ...
    def OpenWithMyPixer(self):
        mw_cmd = "python3 /home/max/Projects/BiblicalOCR/ViewController/Application/0-MainUI/MyPixer.py"
        print(mw_cmd)
        os.system(mw_cmd)

    def OpenWithMyWriter(self):
        mw_cmd = "python3 /home/max/Projects/BiblicalOCR/ViewController/Application/0-MainUI/MyWriter.py"
        print(mw_cmd)
        os.system(mw_cmd)
