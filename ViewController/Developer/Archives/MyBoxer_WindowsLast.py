...
from PyQt5.QtWidgets import  QSpinBox, QTableWidget, QRubberBand, QWidget, QHBoxLayout, QSizeGrip
...
class MainWindow(qtw.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ui.actionDraw_Selected_LineBox_tb.triggered.connect(self.drawLineBox)
        ...
        self.origpixmap = qtg.QPixmap(None)
    ...
    def actionRenameGreek_text_lines_old(self):
        ...
        tr.text2groundtruth(r"c:/users/max/Projects/BiblionOCR/Model/Text/EstablishTruth/Greek/txt_greek_lines_autosplit/greek_book_40_Matthew/", "c:/users/max/Projects/BiblionOCR/Model/Text/EstablishTruth/Greek/txt_greek_lines_autosplit/greek_book_40_Matthew/")
    ...
    def selectBookCombo(self):
        ...
        if self.ui.bookComboBox.currentText() != oldbookabbr:
            ...
            jsonfile = 'c:/users/max/Projects/BiblionOCR/Model/Project/Data/json/BoxerSession.json'
    ...
    def loadImage(self):
        imgdir = "c:/users/max/Projects/BiblionOCR/Model/Project/Images/Workflow"     
    ...
    def setFwdImgDirIterator(self):
        ...
        # val = 400
        # my_list[(my_list.index(val) + 1) % len(my_list)] 
        # next = 100
    ...
    def nextImage(self):
        if self.imgpath:
            try:
                self.imgpath = next(self.imgdirIterator)
                self.ui.ImageLe.setText(os.path.basename(self.imgpath))
                self.getImage(self.imgpath)
            except:
    ...
    def nextText(self):
        if self.textpath:
            try:
                self.textpath= next(self.txtdirIterator)
    ...
    def prevText(self):
        if self.textpath:
            try:
                self.textpath= next(self.txtdirRevIterator)
                self.ui.TextFileName.setText(os.path.basename(self.textpath))
                self.getText(self.textpath)
            except:
                # the iterator has finished, restart it
    ...
    def OpenProjectExplorer(self):
        mw_cmd = "python3 c:/users/max/Projects/BiblionOCR/ViewController/Application/0-MainUI/ProjectBrowser.py"
        print(mw_cmd)
        os.system(mw_cmd)
        '''newapp = qtw.QApplication([])
        dirPath = r'c:/users/max/Projects/BiblionOCR/'
    ...
    def loadcharboximage(self):
        ...
        self.charboxcsvdir = "c:/users/max/Projects/BiblionOCR/Model/Project/Data/csv/"
        ...
        # use a truetype font
        font = ImageFont.truetype("c:/users/max/Projects/BiblionOCR/ViewController/Application/0-MainUI/fonts/FROMVS.ttf", 8) 
        ...
        self.charboximagepath = f"c:/users/max/Projects/BiblionOCR/Model/Images/Complete/Greek/tif_greek_box/{filename}_charbox.tif"
    ...
    def loadwordboximage(self):
        ...
        self.wordboxcsvdir = "c:/users/max/Projects/BiblionOCR/Model/Project/Data/csv/"
        ...
        self.wordboximagepath = f"c:/users/max/Projects/BiblionOCR/Model/Images/Complete/Greek/tif_greek_box/{filename}_wordbox.tif"
    ...
    def on_selection(self):
        ...
        popup.setText("The BoxTable linebox has been selected")
        popup.setInformativeText("Use mouse to Draw/Redraw the selected linebox")
        popup.setStandardButtons(qtw.QMessageBox.Ok)
        answer = popup.exec_()
        if answer == qtw.QMessageBox.Ok:
            pass
    ...
    def on_selectionChanged(self):
        ...
        x = int(self.ui.BoxTable.item(self.row_selected,1).text())
        y = int(self.ui.BoxTable.item(self.row_selected,2).text())
        w = int(self.ui.BoxTable.item(self.row_selected,3).text())
        h = int(self.ui.BoxTable.item(self.row_selected,4).text())

        # overwrite selected row/linebox red rectangle to white
        cv2.rectangle(self.norm,(x,y),(x+w, y+h),(255,255,255),2)
        #column = self.ui.BoxTable.currentColumn()

        self.saveLineBoxImage()
        #imglineboxfile = imgfilename + "_linebox" + imgfileext
        #boximgpath = dest_of_imglinebox + imglineboxfile
        self.ui.ImageLe.setText(self.imglineboxfile)
        
        print("Editing BoxTable selection")
        self.showImage(self.boximgpath)
        self.ui.ZoomComboBox.setCurrentText('11 %')
        
        #if self.row_selected == self.row_current:
        for column in range(1,5):
            ...
            self.ui.BoxTable.setCellWidget(self.row_selected,column,self.spinbox)

            #self.spinbox.valueChanged.connect(on_valueChanged)
        
        self.ui.BoxTable.setSortingEnabled(True)
        self.on_selection()

    '''
    def updateBoxFiles(self):
        
        # Save edits to BoxText
        self.BoxTable2csv()

        # Reload and normalize source image
        self.showImage(self.imgpath)
        self.normImage()

        # Regenerate LineBox Image 
    ...
    def update_BoxText(self,line,x,y,w,h):
        print(f'From drawLineBox method - Line: {line}  X: {x}  Y: {y}  W: {w} H: {h}')
        ...
        # Update BoxText with edited linebox dimensions with csv.reader keyed to selected BoxTable Line number
        '''print(f'Updating BoxText File: {self.txtpath}')
        csvfilename = self.txtpath
        tempfile = NamedTemporaryFile(mode='w', delete=False)
        fields = ['Line', 'X', 'Y', 'W', 'H', 'X+W', 'Y+H']
        ...
        #with open(csvfilename, 'r') as csvfile:
            ...
            for row in reader:
                if row['Line'] == self.line:
                    print('updating row', row['Line'])
                    row['Line'], row['X'], row['Y'], row['W'], row['H'], row['X+W'], row['Y+H'] = x,y,w,h,x+w,y+h
                    row = {'Line': row['Line'], 'X':row['X'], 'Y': row['Y'], 'W': row['W'], 'H': row['H'], 'X+W': row['X+W'], 'Y+H': row['Y+H']}
                    print(f'Updating BoxText File Row {row}')
                    writer.writerow(row)
        shutil.move(tempfile.name, csvfilename)
        self.ui.BoxText.clear()
        self.showText(csvfilename)'''

    def update_BoxTable(self):
        # Update selected row in BoxTable from updated BoxText
        self.ui.BoxTable.setSortingEnabled(False)
        reader = list(csv.reader(self.txtpath, delimiter="\t"))
        # Remove column header from list of rows
        lineboxes = reader[1:]
        #self.rowCount = len(lineboxes)
        col_list = []
        for linebox in lineboxes:
            ...
            if line == str(self.row_selected):
                ...
                col_list.append(x,y,w,h,xw,yh)         
                column = 1
                for value_str in col_list:
                    #use this to update values if item is qtw.QTableWidgetItem()
                    #item = qtw.QTableWidgetItem()
                    #item.setText(value_str)
                    #self.ui.BoxTable.setItem(self.row_selected, column, item)
                    self.tableitem = self.ui.BoxTable.item(self.row_selected, column)
                    cellwidget = self.ui.BoxTable.cellWidget(self.row_selected, column)
                    print(f'Selected Cell Location:  Row: {self.row_selected} Column: {column}')
                    print(f'Current Cell Widget: {cellwidget}')
                    ...
                    column += 1
                '''
                # Copied from on_selectionChanged() above
                for column in range(1,5):
                    self.tableitem = self.ui.BoxTable.item(self.row_selected, column)
                    cellwidget = self.ui.BoxTable.cellWidget(self.row_selected, column)
                    cellvalue = self.ui.BoxTable.item(self.row_selected,column).text()
    ...
    def on_valueChanged(self):
        self.ui.BoxTable.setSortingEnabled(False)
        print(f"Spinbox Value Changed To: {str(self.spinbox.value())}")
        row = self.ui.BoxTable.currentRow()
        column = self.ui.BoxTable.currentColumn()

        self.ui.BoxTable.item(row,column).setText(str(self.spinbox.value()))
        ...
        self.ui.BoxTable.resizeColumnsToContents()
        self.ui.BoxTable.closePersistentEditor(self.tableitem)
        self.ui.BoxTable.setSortingEnabled(True)
        self.updateBoxFiles()
    '''
    
    def drawLineBox(self):
        ...
        DrawImg_mqrect = QRect(DrawImg_xs,DrawImg_ys,DrawImg_ws,DrawImg_hs)
        print("Offset Scaled QRect = " + str(DrawImg_mqrect))
        
        # Upscale DrawImg QRect at RefImg MainWindow origin(0,0)
        ...
        self.update_BoxTable()
        #self.on_deselection()
        # Save Line Image
        #roi = self.norm[y:y+h, x:x+w]
        #self.saveimgline(roi,self.line)

        # Regenerate and Save LineBox Image 
        #self.drawLineBoxImage()
        
        #self.saveLineBoxImage()

        # Reload modified LineBox image
        #self.showImage(self.boximgpath)
    ...
    def linebox_edit_split(self):
        ...
        print(f'Text File Name: {self.txtfilename}  Image File Name: {self.imgfilename}')
        if self.txtfilename == self.imgfilename:
            self.getText(self.txtpath)
            #BoxText2BoxTable()
            self.drawLineBoxImage()
            self.saveLineBoxImage()
            ...
            print(f'The linebox text: {self.txtfilename} does not match the linebox image: {self.imgfilename} -- Please try again!')
