...
from PyQt5.QtWidgets import  QSpinBox, QListWidget, QListWidgetItem, QRubberBand, QWidget, QHBoxLayout, QSizeGrip
...
class MainWindow(qtw.QMainWindow):
    ...
    def on_selection(self):
        ...
        popup.setText("Select a linebox drawing method")
        #popup.setInformativeText("Select a method for modifying the selected linebox")
        
        mouseButton = popup.addButton('Mouse', qtw.QMessageBox.YesRole)
        mouseIcon = qtg.QIcon()
        mouseIcon.addPixmap(qtg.QPixmap("/home/max/Projects/BiblionOCR/ViewController/Application/0-MainUI/Icons/draw-rectangle-diagonally-16.png"), qtg.QIcon.Normal, qtg.QIcon.Off)
        mouseButton.setIcon(mouseIcon)
        spinboxButton = popup.addButton('SpinBoxes', qtw.QMessageBox.YesRole)
        spinboxIcon = qtg.QIcon()
        spinboxIcon.addPixmap(qtg.QPixmap("/home/max/Projects/BiblionOCR/ViewController/Application/0-MainUI/Icons/table-small.png"), qtg.QIcon.Normal, qtg.QIcon.Off)
        spinboxButton.setIcon(spinboxIcon)
        ...
        cancelIcon.addPixmap(qtg.QPixmap("/home/max/Projects/BiblionOCR/ViewController/Application/0-MainUI/Icons/cross-small.png"), qtg.QIcon.Normal, qtg.QIcon.Off)
        cancelButton.setIcon(cancelIcon)
        popup.exec()
        if popup.clickedButton() == mouseButton:
            do_Rb()
        elif popup.clickedButton() == spinboxButton:
            do_Sb()
    ...
    def onValueChanged(self):
        ...
        xw = self.xval+self.wval
        yh = self.yval+self.hval
        print(f'Line: {str(line)} X:{str(self.xval)} Y:{str(self.yval)} W:{str(self.wval)} H:{str(self.hval)} X+W: {str(xw)} Y+H: {str(yh)}')
        #self.resetLineBox()
    ...
    def getSbLineBox(self):
        ...
        def onSpinboxValueChanged():
                #spinner = self.ui.BoxTable.focusWidget()
                print('This will be the handler for the indexed QModelIndex of the spinner that is changed')
                self.onValueChanged()
        self.ui.BoxTable.setSortingEnabled(False)
        #if self.mode == "SbEdit":
        if self.mode == "SbEdit":
            for column in range(1,5):
                ...
                spinbox = QSpinBox(cellwidget) #changed parent from None to self.ui.BoxTable - could also be just self
                spinbox.setMaximum(6000)
                spinbox.setValue(int(cellvalue))
                #listWidgetItem = QListWidgetItem(str(spinbox.value()))
                #print(f'listWidgetItem: {listWidgetItem}')
                #listWidget.addItem(listWidgetItem)
                self.ui.BoxTable.setCellWidget(self.row_selected,column,spinbox)
                #listWidget.itemChanged.connect(self.onValueChanged)
                spinbox.valueChanged.connect(onSpinboxValueChanged)
            #self.ui.BoxTable.openPersistentEditor(listWidget)
            

        self.ui.BoxTable.setSortingEnabled(True)            
        '''
        # Reconnect put signal
        #self.ui.actionDraw_Table_LineBox_tb.triggered.connect(self.putSbLineBox(x,y,w,h))
        '''
    # Edit LineBox Method
