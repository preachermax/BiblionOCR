...
while loopcount <= lastline:   
    cursor_FR.execute("SELECT Line,BookNum,Book,Chapter,Verse,Word,UnicodeWord,StrongOpen,Strong,StrongClose,RMACOpen,RMAC,RMACClose,WordPunc FROM Bible WHERE Line = " + str(loopcount))
    ...
    for vword in vwords:
        ...
        wordgroup = str(uniword) + str(punc) + " " + str(strongopen) + str(strong) + str(strongclose) + str(RMACopen) + str(RMAC) + str(RMACclose)
