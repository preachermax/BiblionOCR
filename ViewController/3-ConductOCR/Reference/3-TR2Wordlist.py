...
path_of_textfile = path_of_text + "TR-linestest.txt"
...
with open(path_of_text + "TR-stripped.txt", 'w') as file:
    file.write(strippedfile)
    file.close()

with open(path_of_text + "TRWordlistcount.txt", 'w', newline="") as file:
    writer = csv.writer(file, delimiter="\t")

    for w in open(path_of_text + "TR-stripped.txt").read().split():
        word_counts[w] += 1
...
with open(path_of_text + "TRWordlistsort.txt", 'w', newline="") as file:
    writer = csv.writer(file, delimiter="\t")
    
    wordlist_csv = open(path_of_text + "TRWordlistcount.txt", 'r')
    ...
    with open(path_of_text + "TRWordlist.txt", 'w', newline="") as file:
        writer = csv.writer(file, delimiter="\t")   
        for k in d.keys():  
