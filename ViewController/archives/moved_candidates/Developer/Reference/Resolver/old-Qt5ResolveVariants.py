...
def loadTableView(rowid):
    helper = SqliteHelper("/home/max/Projects/Python/SQLite/FROMVS.db")
...
def updatebible():
    ...
    for variant in variants:
        if variant[2]:    
            ...
            '''englishquery = "SELECT English FROM Bible WHERE Line = ? AND NoDiaWord = ?"
            engdata = (line,varword)
            englishtup = helper.selectone(englishquery,engdata)
