...
from PyQt5.QtWidgets import QStyledItemDelegate, QWidget, QStyleOptionViewItem, QSpinBox, QTableWidget
...
class MainWindow(qtw.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ui.actionEdit_Line_Boxfiles_tb.triggered.connect(self.linebox_edit_split)
        ...
        self.ui.LineBoxImagebutton.clicked.connect(self.linebox_make_split)
    ...
    def get_session_settings(self):
        ...
        with open('/home/max/Projects/BiblionOCR/Model/Project/Data/json/BoxerSession.json') as f:
    ...
    #def actionlinebox_make_split(self):
    ...
    def actionRenameGreek_text_lines_old(self):
        ...
        tr.text2groundtruth(r"/home/max/Projects/BiblionOCR/Model/Text/EstablishTruth/Greek/txt_greek_lines_autosplit/greek_book_40_Matthew/", "/home/max/Projects/BiblionOCR/Model/Text/EstablishTruth/Greek/txt_greek_lines_autosplit/greek_book_40_Matthew/")
    ...
    def initBookCombo(self):
        ...
        # Opening JSON file
        with open('/home/max/Projects/BiblionOCR/Model/Project/Data/json/BooksAbbrName.json') as f:
            # returns JSON object as
            # a dictionary
            data = json.load(f)
    ...
    def selectBookCombo(self):
        ...
        if self.ui.bookComboBox.currentText() != oldbookabbr:
            ...
            jsonfile = '/home/max/Projects/BiblionOCR/Model/Project/Data/json/BooksMarkDown.json'
            ...
            jsonfile = '/home/max/Projects/BiblionOCR/Model/Project/Data/json/BoxerSession.json'
    ...
    def loadImage(self):
        imgdir = "/home/max/Projects/BiblionOCR/Model/Project/Images/Workflow"     
    ...
    def loadText(self):
        ...
        self.txtpath = qtw.QFileDialog.getOpenFileName(
            self.ui.centralwidget, 'Open text file', '/home/max/Projects/BiblionOCR/Model/Project/Text/EstablishTruth/Greek/txt_greek_lines_autosplit/','Text files (*.txt)')[0]
    ...
    def nextText(self):
        ...
        if self.txtfileList:
            try:
                ...
                #self.txtpath = os.path.join(self.dirname, "/",self.txtfilename)
    ...
    def OpenProjectExplorer(self):
        mw_cmd = "python3 /home/max/Projects/BiblionOCR/ViewController/Application/0-MainUI/ProjectBrowser.py"
        print(mw_cmd)
        os.system(mw_cmd)
        '''newapp = qtw.QApplication([])
        dirPath = r'/home/max/Projects/BiblionOCR/'
    ...
    def loadcharboximage(self):
        ...
        self.charboxcsvdir = "/home/max/Projects/BiblionOCR/Model/Project/Data/csv/"
    ...
    def loadwordboximage(self):
        ...
        self.wordboxcsvdir = "/home/max/Projects/BiblionOCR/Model/Project/Data/csv/"
    ...
    #def linebox_make_split(self, imagepath, linebox, splitdir, splitlines):
    def linebox_make_split(self):
            dest_of_imgautosplit = self.greeklinesautosplit + r"/"
            dest_of_imglinebox = self.greeklinesbox + r"/" + self.greekbookmarkdown + r"/"
            path_of_image = self.imgpath
            dest_of_txtlinebox = self.txtgreeklinebox + r"/" + self.greekbookmarkdown + r"/"
            dosplit = True
            '''
            print(f"dest_of_autosplit: {dest_of_imgautosplit}")
            print(f"dest_of_imglinebox: {dest_of_imglinebox}")
            print(f"path_of_image: {path_of_image}")
            print(f"dest_of_txtlinebox: {dest_of_txtlinebox}")'''
            
            def saveimgline(roi,bnum):
                PILimage = Image.fromarray(roi)
                thresh = 127
                fn = lambda x : 255 if x > thresh else 0
                PIL_BWimage = PILimage.convert('L').point(fn, mode='1')
                tif_outfile = dest_of_imgautosplit + filename + "_Line" + str(bnum) + ".tif"
                print("Generating: " + tif_outfile)
                PIL_BWimage.save(tif_outfile, "TIFF", dpi=(300,300))    

            def saveLineBoxImage():
                imgboxfile = dest_of_imglinebox + filename + "_linebox" + fileext
                cv2.imwrite(imgboxfile,norm)
                self.imgpath = imgboxfile
                self.showImage(self.imgpath) 

            def BoxText2BoxTable():
            ...
            def BoxTable2csv():
                self.txtpath = dest_of_txtlinebox + filename + "_linebox.txt"
            ...
            filestr = os.path.basename(path_of_image)
    ...
    def linebox_edit_split(self):
        path_of_image_page = self.greekpages
        dest_of_imgautosplit = self.greeklinesautosplit + r"/"
        dest_of_imglinebox = self.greeklinesbox + r"/" + self.greekbookmarkdown + r"/"
        path_of_txtlinebox = self.txtgreeklinebox + r"/" + self.greekbookmarkdown + r"/"
        ...
        imgfilestr = os.path.basename(imgpath)
        imgfilesplit = os.path.splitext(imgfilestr)
        imgfilename = imgfilesplit[0]
        imgfileext = imgfilesplit[1]
        self.ui.ImageLe.clear()
        self.ui.ImageLe.setText(imgfilename + "_linebox.tif")
        ...
        # Get matching txtlinebox file
        self.txtpath = path_of_txtlinebox + imgfilename + "_linebox.txt"
        txt = self.txtpath
        txtfilestr = os.path.basename(txt)
        txtfilesplit = os.path.splitext(txtfilestr)
        txtfilename = txtfilesplit[0]
        txtfilename = txtfilename.replace("_linebox","")
        txtfileext = txtfilesplit[1]
        self.ui.TextLE.clear()
        self.ui.TextLE.setText(txtfilestr)
        
        def drawLineBoxImage():
            with open(txt, "r") as txtboxfile:
            ...
            txtboxfile.close()
        def rowSelected():
            print(f'Selected row: {self.ui.BoxTable.currentRow()}')
            print(f'Selected Cell Location:  Row: {self.ui.BoxTable.currentRow()} Column: {self.ui.BoxTable.currentColumn()}')

        def editBoxTable(line,roi):
            # Modify to edit a specific roi when a line is selected in BoxTable
            
            self.ui.BoxTable.setSortingEnabled(False)
            self.ui.BoxTable.setItemDelegate(SpinBoxDelegate())
            for row in range(self.ui.BoxTable.rowCount()):
                for col in range(self.ui.BoxTable.columnCount()):
                    it = self.ui.BoxTable.item(row, col)
                    if it is None:
                        it = qtw.QTableWidgetItem()
                        self.ui.BoxTable.setItem(row, col, it)
                    self.ui.BoxTable.openPersistentEditor(it)
            self.ui.BoxTable.resizeColumnsToContents()
            self.ui.BoxTable.setSortingEnabled(True)
            
            ####  Keep this for writing linebox edits back to linebox image file!                  
            #cv2.imwrite(imgpath, norm)
        ...
        def saveimgline(roi,bnum):
            ...
            fn = lambda x : 255 if x > thresh else 0
            PIL_BWimage = PILimage.convert('L').point(fn, mode='1')
            tif_outfile = dest_of_imgautosplit + imgfilename + "_Line" + str(bnum) + ".tif"
            print("Generating: " + tif_outfile)
            PIL_BWimage.save(tif_outfile, "TIFF", dpi=(300,300))    

        def saveLineBoxImage():
            imgboxfile = dest_of_imglinebox + imgfilename + "_linebox" + imgfileext
            cv2.imwrite(imgboxfile,norm)
            self.imgpath = imgboxfile
            self.showImage(self.imgpath) 
        ...
        def BoxTable2csv():
            self.txtpath = path_of_txtlinebox + txtfilename + "_linebox.txt"
            print(f'Path of linebox.txt: {self.txtpath}')
            if os.path.exists(self.txtpath):
                print(f'Removing: {self.txtpath}')
                os.remove(self.txtpath)
    ...
    def oldermakelineboximage(self):
        ...
        self.lineboxcsvdir = "/home/max/Projects/BiblionOCR/Model/Project/Data/csv/"
                ...
                if len(b)==12:
                    ...
                    if imglinenum == linenum:
                        ...
                        with open(lineboxcsvpath, mode='a') as file_:
                            ...
                            file_.write("\n")  # Next line.
                        self.ui.BoxText.insertPlainText(f"{imglinenum}\t{imgwordnum}\t{imgword}\t{xmin}\t{liney}\t{wmax}\t{liney + lineheight}\n")
                        #linenum += 1
    ...
    def older2loadlineboximage(self):
        '''Load Word Box Image'''
        img = cv2.imread(self.imgpath)
        hImg, wImg,_ = img.shape
        self.imgdir = os.path.dirname(self.imgpath)
        ...
        #lines = []
        lineboxes = pytesseract.image_to_data(img,lang="feg")
        #self.ui.BoxText.insertPlainText(lineboxes)
        linenum = 1
        #prevx = 0
        prevy = hImg
        #prevw = 0
