...
for wline in wlines:
        ...
        #cursor_TW.execute("SELECT DISTINCT ID, Line, WordNum, Word, NoDiaWord, NoDiaLemma, Strong, RMAC, Lemma FROM TRaBible WHERE Line=?", (flinenum,))
        ...
        for tword in twordloop:
                ...
                if fnodiaword == tnodiaword or fnodiaword == tnodialemma and fstrong != tstrong and frmac != trmac:
                        # monitor data output
                        print(twordnum," ", tnodiaword," ", tnodialemma," ", fwordnum," ", fnodiaword," ", fstrong," ", frmac," ", flemma)
                        
                        # update database
                        sql_qry = '''UPDATE Bible SET Strong = ?, RMAC = ?, Lemma = ?, NoDiaLemma = ? WHERE ID = ?'''
                        data = (tstrong, trmac, tlemma, tnodialemma, id)
