...
varui = uic.loadUi("Workflow/4-PostProcess/Erasmvs/VariantsMainForm.ui")
...
def main():
    ...
    varui.CriticalCkBox.stateChanged.connect(VarCodeBuild)
    ...
    varui.UpdateAllButton.clicked.connect(updateall)
    varui.ResolveButton.clicked.connect(resolved)
    varui.unresRadButton.clicked.connect(loadTableView)
    varui.resRadButton.clicked.connect(loadTableView)
    varui.allRadButton.clicked.connect(loadTableView)

def loadTableView(rowid):
    helper = SqliteHelper("/home/max/Projects/Python/SQLite/TRa-Words_FROMVS.db")
    
    varui.VarianceTable.setRowCount(0)
    
    if  varui.resRadButton.isChecked():
        variants = helper.select("SELECT ID,Line,Book,Chapter,Verse,WordNum,Word,NoDiaWord,VarWord,Strong,RMAC,Lemma,ErrorCode,VarCode,Description,Preserved,Critical,Error,Variance,Context,Inflection,Resolved FROM Variants WHERE Resolved = '1'")  
    elif varui.unresRadButton.isChecked():
        variants = helper.select("SELECT ID,Line,Book,Chapter,Verse,WordNum,Word,NoDiaWord,VarWord,Strong,RMAC,Lemma,ErrorCode,VarCode,Description,Preserved,Critical,Error,Variance,Context,Inflection,Resolved FROM Variants WHERE Resolved = '0'")
    else:
        variants = helper.select("SELECT ID,Line,Book,Chapter,Verse,WordNum,Word,NoDiaWord,VarWord,Strong,RMAC,Lemma,ErrorCode,VarCode,Description,Preserved,Critical,Error,Variance,Context,Inflection,Resolved FROM Variants")
...
def loadVarWordsCombo():
    helper = SqliteHelper("/home/max/Projects/Python/SQLite/TRa-Words_FROMVS.db")
    varwords = helper.select("SELECT DISTINCT NoDiaWord FROM TRaBible ORDER BY NoDiaWord")
    #print(varwords)
...
def selectVarWordsCombo():
    ...
    helper = SqliteHelper("/home/max/Projects/Python/SQLite/TRa-Words_FROMVS.db")
    varwords = helper.select("SELECT DISTINCT NoDiaWord,Strong,RMAC,Lemma FROM TRaBible WHERE NoDiaWord =" + "'" + selvarword + "'")
...
def resolved():
    conn_TR = sqlite3.connect('/home/max/Projects/Python/SQLite/TRa-Words_FROMVS.db')
    ...
    for vline in vlines:
        ...
        insert_parameters = """INSERT OR REPLACE INTO Resolved (Line,Book,Chapter,Verse,WordNum,Word,NoDiaWord,VarWord,
                                ...
                                Preserved,Critical,Error,Variance,Context,Inflection,Resolved)
                                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"""
        insert_data = (line,book,chapter,verse,wordnum,word,nodiaWord,varword,strong,rmac,lemma,varform,vartype,impactcode,errorcode,varcode,desc,preserved,critical,error,variance,context,inflection,resolved)
...
def VarCodeBuild():  
    ...
    if varui.CriticalCkBox.isChecked():
        f2 = "C"
    else:
        f2 = ""
...
def updateone():   
    ...
    query = """UPDATE Variants SET VarWord = ?,Strong = ?,RMAC = ?,Lemma = ?,VarCode = ?, ErrorCode = ?, Description = ?, VarianceForm = ?, VarianceType = ?, ImpactCode = ?, Preserved = ?, Critical = ?, Error = ?, Variance = ?, Context = ?, Inflection = ?, Resolved = ?
                            WHERE Line = ? and WordNum = ?"""
    data = (varword,strong,rmac,lemma,varcode,errorcode,desc,varform,vartype,impactcode,preserved,critical,error,variance,context,inflection,resolved,line,wordnum)
    #startProgressBar()
...
def updatesim():
    ...
    query = """UPDATE Variants SET VarWord = ?,Strong = ?,RMAC = ?,Lemma = ?,VarCode = ?, ErrorCode = ?, Description = ?, VarianceForm = ?, VarianceType = ?, ImpactCode = ?, Preserved = ?, Critical = ?, Error = ?, Variance = ?, Context = ?, Inflection = ?, Resolved = ?
                            WHERE NoDiaWord = ?"""
    data = (varword,strong,rmac,lemma,varcode,errorcode,desc,varform,vartype,impactcode,preserved,critical,error,variance,context,inflection,resolved,nodiaword)
    helper.update(query,data)
    rowid = varui.VarianceTable.currentRow()
    loadTableView(rowid)
    loadFormView()
...
def updateall():
    ...
    query = """SELECT Line,WordNum,VarWord,Strong,RMAC,Lemma,VarCode FROM Resolved"""
    ...
    for variant in variants:
        if variant[2]:
            ...
            query = """UPDATE Bible SET VarWord = ?,Strong = ?,RMAC = ?,Lemma = ?,VarCode = ?
                                    WHERE Line = ? and WordNum = ?"""
            data = (varword,strong,rmac,lemma,varcode,line,wordnum)
            helper.update(query,data)
...
def loadFormView():
    ...
    if varui.VarianceTable.item(getSelectedRowId(),15).text() == "1":
        varui.PreservedCkBox.setChecked(True)
    else:
        varui.PreservedCkBox.setChecked(False)
    if varui.VarianceTable.item(getSelectedRowId(),16).text() == "1":
        varui.CriticalCkBox.setChecked(True)
    else:
        varui.CriticalCkBox.setChecked(False)
    if varui.VarianceTable.item(getSelectedRowId(),17).text() == "1":
        varui.ErrorCkBox.setChecked(True)
    else:
        varui.ErrorCkBox.setChecked(False)
    if varui.VarianceTable.item(getSelectedRowId(),18).text() == "1":
        varui.VarianceCkBox.setChecked(True)
    else:
        varui.VarianceCkBox.setChecked(False)
    if varui.VarianceTable.item(getSelectedRowId(),19).text() == "1":
        varui.ContextCkBox.setChecked(True)
    else:
        varui.ContextCkBox.setChecked(False)
    if varui.VarianceTable.item(getSelectedRowId(),20).text() == "1":
